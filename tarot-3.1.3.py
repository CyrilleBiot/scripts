...
dico_contrat['garde contre'] = 25 * 4
...
def updateScore(gain,  preneur,  liste_des_primes):
    # Mise à jour du score
    # Ajout des scores
    print(dico_primes)

    # Si le contrat est rempli, on additionne les gains
    # Sinon on les soustrait
    if gain > 0:
        gain_tour = gain + dico_contrat[contrat_tour]
    else:
        gain_tour = gain - dico_contrat[contrat_tour]
# ######################################"
    # TEST Petit au bout pour le preneur (prime multipliable)
    if 'petit au bout' in liste_des_primes[0]:
        if liste_des_primes[0][0] == preneur:
            print('Prime "Petit au bout" pour le preneur "{}" : +10 '. format(liste_des_primes[0][0]))
            gain_tour = gain_tour + 10
        else:
            print('Prime "Petit au bout" pour  "{}" (+10) ; preneur {} :'. format(liste_des_primes[0][0],preneur))
            gain_tour = gain_tour - 10
    else:
        print('pas de petit au bout')
# ######################################"
    ...
    for nom in dico_joueurs:
        ...
        if nom == preneur:
            score = score + gain_tour * (nb_joueurs - 1)
        else:
            score = score - gain_tour
        dico_joueurs[nom].append(score)

    # GESTION DES PRIMES
    print("Dico des primes : " , dico_primes)
    print("Liste des primes : ", liste_des_primes)

    i = 0
    for nom in liste_des_primes:
        print(nom)
        if nom[0] == liste_des_primes[i][0]:
            bonus = dico_primes[liste_des_primes[i][1]]
            print(nom, bonus)
        else:
            bonus = dico_primes[liste_des_primes[i][1]] * -1
            print(nom, bonus)
        i += 1
    print(dico_joueurs)
...
# =====================================================================================================================
fin_du_jeu = ''
while fin_du_jeu.lower() not in ['quitter', 'q']:
    ...
    fin_du_jeu = input("Cesser la partie ? Saisir 'Quitter' ou 'q' : ")

else:
    print('Bien joué ! A la prochaine ;).')